...
const videoConstraints = {
	width: { min: 800, ideal: 1280, max: 2560 }, // High-definition capture
...
export default function App() {
	...
	return (
		<div className='min-h-screen w-screen bg-[#0A0A0C] text-zinc-100 flex flex-col items-center py-12 px-4'>
			<div className='w-full max-w-2xl flex flex-col items-center text-center space-y-8'>
				...
				<div className='relative w-full aspect-video rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl bg-black'>
					<Webcam
						ref={webcamRef}
						audio={false}
						screenshotFormat='image/jpeg'
						videoConstraints={videoConstraints}
						className='w-3/4 h-3/4 object-cover mx-auto mt-12 rounded-lg'
					/>
				...
				<div className='w-full flex items-center gap-4 bg-zinc-900/90 backdrop-blur-xl p-3 rounded-[2rem] border border-white/10 shadow-2xl'>
					<button
						title='Upload'
						onClick={() => fileInputRef.current?.click()}
						className='p-4 text-zinc-400 hover:text-white hover:bg-white/5 rounded-2xl transition-all'
					>
						<Upload size={24} />
					</button>
